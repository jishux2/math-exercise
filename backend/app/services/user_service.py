...
class UserService(BaseService[User, UserCreateBase, UserUpdate]):
    ...
    def get_parent_stats(self, parent_id: int) -> dict:
        ...
        for child in children:
            ...
            child_total_score = sum(ex.score for ex in child_exercises) if child_exercises else 0
            child_avg_score = round(child_total_score / child_total_exercises, 2) if child_total_exercises > 0 else 0
        ...
        # 计算总体平均正确率
        average_accuracy = round(total_score / total_exercises, 2) if total_exercises > 0 else 0
        
        # 获取最近活动
        recent_activities = (
            self.db.query(Exercise, User)
            .join(User, Exercise.student_id == User.id)
            .filter(Exercise.student_id.in_([child.id for child in children]))
            .order_by(Exercise.completed_at.desc())
            .limit(5)
            .all()
        )
